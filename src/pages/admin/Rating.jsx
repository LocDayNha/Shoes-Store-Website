...
const Rating = () => {
  ...
  useEffect(() => {
    const fetchRatings = async () => {
      try {
        const response = await axiosClient.get('/rating/get-all-ratings');
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'idOder.createdAt',
      key: 'idOder.createdAt',
      render: (text, record) => (
        <span>{moment(record?.idOder?.createdAt).format('YYYY-MM-DD - HH:mm:ss')}</span>
      ),
    },
    {
      title: 'Tùy chọn',
  ...
  const handleOk = async (record) => {
    try {
      ...
      await axiosClient.delete(`/rating/delete-ratings/${record._id}`);
