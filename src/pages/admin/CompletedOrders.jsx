...
const CompletedOrders = () => {
  ...
  const completedOrders = orders.filter(
    (order) => order.status === OrderStatusEnum.COMPLETED.value
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      //   <div style={{ padding: 8 }}>
      //     <Input
      //       placeholder={`Search Người đặt hàng`}
      //       value={selectedKeys[0]}
      //       onChange={(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])}
      //       onPressEnter={() => handleSearch(selectedKeys, confirm, ['userId', 'name'])}
      //       style={{ marginBottom: 8, display: 'block' }}
      //     />
      //     <Space>
      //       <Button
      //         type="primary"
      //         onClick={() => handleSearch(selectedKeys, confirm, ['userId', 'name'])}
      //         icon={<SearchOutlined />}
      //         size="small"
      //         style={{ width: 90 }}
      //       >
      ...
      // ),
      // filterIcon: (filtered) => (
      //   <SearchOutlined style={{ color: filtered ? '#1677ff' : undefined, fontSize: '20px' }} />
      // ),
      // onFilter: (value, record) =>
      //   record['userId.name'] && record['userId.name'].toLowerCase().includes(value.toLowerCase()),
      // render: (text) =>
      //   searchedColumn === ? (
      //     <Highlighter
      //       highlightStyle={{ backgroundColor: 'orange', padding: 0 }}
      //       searchWords={[searchText]}
      //       autoEscape
      //       textToHighlight={text ? text.toString() : ''}
      //     />
